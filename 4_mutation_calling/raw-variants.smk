...
rule all:
    input: config["final_output"]
        
# variant discovery, parallelized across regions of the genome
rule freebayes_interval:
    input:
        ref=config["genome"],
        short_bam=["data/merged/{}.bam".format(x) for x in units.index.levels[0]],
        short_bai=["data/merged/{}.bam.bai".format(x) for x in units.index.levels[0]],
